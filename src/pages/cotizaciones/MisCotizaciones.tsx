...
export function MisCotizaciones() {
  ...
  // Función para obtener el estado de una cotización basado en el servicio
  const getQuoteStatus = (quote: typeof quotes[0]): 'pendiente' | 'aceptada' | 'rechazada' => {
    const service = services.find(s => s.id === quote.serviceId);
    if (!service) return 'pendiente';

    // Si el servicio está asignado
    if (service.status === 'asignado' || service.status === 'completado') {
      // Si esta cotización fue la seleccionada
      if (service.assignedQuoteId === quote.id) {
        return 'aceptada';
      }
      // Si otra cotización fue seleccionada
      return 'rechazada';
    }

    // Si el servicio está en publicado o en evaluación
    return 'pendiente';
  };
            ...
            <div className="flex items-start justify-between">
              <div>
                ...
                <p className="text-2xl text-slate-900">
                  {myQuotes.filter(q => getQuoteStatus(q) === 'pendiente').length}
                </p>
              </div>
          ...
          <CardBody>
            <div className="flex items-start justify-between">
              <div>
                ...
                <p className="text-2xl text-slate-900">
                  {myQuotes.filter(q => getQuoteStatus(q) === 'aceptada').length}
                </p>
              </div>
                  ...
                  <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
                    ...
                    <div className="flex-1">
                      <div className="flex items-center gap-2 mb-2">
                        <Badge variant={statusColors[quoteStatus]}>
                          {statusLabels[quoteStatus]}
                        </Badge>
